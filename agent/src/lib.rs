//! Arna agent core — screen capture + WebRTC streaming + input injection.
//!
...
/// backend, and serve admitted viewers (stream frames + inject their input).
/// `consent` decides whether each requesting console is admitted. Returns when
/// the signaling socket closes.
pub async fn run(url: String, id: String, consent: ConsentFn) {
    ...
    if let Err(e) = p2p::answer_streaming(signaling, id, consent, on_channel).await {
