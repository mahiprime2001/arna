...
// Opened by the Rust side as: index.html?view=bubbleconsent&id=<win>&app=<label>
const params = new URLSearchParams(location.search);
const id = params.get("id") ?? "";
const app = params.get("app") || "an app";
...
async function respond(allow: boolean) {
  ...
  busy.value = true;
  try {
    await invoke("respond_bubble_consent", { id, allow });
  } catch (e) {
    console.error("respond_bubble_consent failed", e);
    busy.value = false;
  }
}
...
<template>
  <main class="card">
    ...
    <p class="kicker">Open an app</p>
    <h1 class="lead">The person connected wants to open <strong>{{ app }}</strong> in a sandbox.</h1>
    <p class="sub">It opens on a separate desktop and they control only it — you keep using your computer normally. It can't see your open apps or files.</p>
...
.card {
  ...
  overflow: hidden;
  background: radial-gradient(120% 80% at 50% -20%, rgba(109, 94, 252, 0.16), transparent 60%), #0e121b;
}
.glow {
  position: absolute;
  inset: -40% 20% auto 20%;
