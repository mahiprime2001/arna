//! Arna — one desktop app that does both sides of a remote session.
//!
//! - **Control others:** the main window is the Vue console (sign in, pick one of
//!   your devices, connect, view + control it).
//! - **Be controlled:** the same process runs the agent loop in the background
//!   (screen capture, input, files, chat, clipboard, multi-monitor) so other
//!   people can connect to *this* PC. A tray icon keeps it alive; an always-on-top
//!   popup asks the operator to Accept/Decline each incoming connection.
//!
//! Closing the main window hides it to the tray (the PC stays reachable); Quit
//! from the tray fully exits. To be reachable, this PC needs a pairing (device id
//! + token from "Add a device"): set via the tray's "Pair this device…", saved to
//! the app config dir, or provided by env vars for dev.
...
// ---------------------------------------------------------------------------
// Pairing (saved credentials so this PC can be reached)
// ---------------------------------------------------------------------------
...
fn ensure_pair_window(app: &AppHandle) {
    ...
    let _ = WebviewWindowBuilder::new(app, "pair", WebviewUrl::App("index.html?view=pair".into()))
        .title("Arna — make this PC reachable")
...
/// Show (and focus) the main console window.
...
/// Save the pasted credentials and bring this PC online. Returns `true` if the
/// agent loop started now, or `false` if it was already running (re-pair needs a
...
/// Send a reply from this PC to the connected console.
#[tauri::command]
async fn send_chat(
    app: AppHandle,
...
fn make_consent(app: AppHandle, pending: Pending) -> ConsentFn {
    Arc::new(move |req: ConnectRequest| {
        ...
        Box::pin(async move {
            ...
            // Pass the request details to the popup via the query string. `view`
            // routes the shared frontend to the consent UI (the default view is
            // the main console).
            let url = format!(
                "index.html?view=consent&from={}&name={}&code={}",
...
fn start_agent(app: &AppHandle, cfg: Pairing) -> bool {
    ...
    // Download: when the controlling console asks for a file, show a native
    // picker so the operator chooses what leaves their PC.
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            // Tray icon: open the console, make this PC reachable, or quit.
            let open = MenuItem::with_id(app, "open", "Open Arna", true, None::<&str>)?;
            let pair = MenuItem::with_id(app, "pair", "Pair this device…", true, None::<&str>)?;
            let quit = MenuItem::with_id(app, "quit", "Quit Arna", true, None::<&str>)?;
            ...
            // Bring this PC online if it's already paired (env for dev, else the
            // saved pairing). With no pairing we stay console-only until the user
            // pairs via the tray — the main window is the primary experience.
        ...
        .expect("error while running Arna");
}
